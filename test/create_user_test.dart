...
void main() {
  ...
  test(
    'Debe crear un usuario (con email aleatorio) en Auth y Firestore, y luego limpiarlo',
    () async {
      ...
      try {
        ...
        expect(
          user,
          isNotNull,
          reason: 'La creación del usuario en Firebase Auth no debería fallar.',
        );
        print('PASO 1 ÉXITO: Usuario creado en Auth con UID: ${user!.uid}');

        print(
          'PASO 2: Añadiendo documento a Firestore para el UID: ${user.uid}',
        );
        ...
        expect(
          docSnapshot.exists,
          isTrue,
          reason: 'El documento del usuario debería existir en Firestore.',
        );
        expect(
          docSnapshot.data()?['role'],
          'cafeteria',
          reason: 'El rol del usuario debería ser "cafeteria".',
        );
        ...
        if (user != null) {
          print(
            'PASO 4: Limpiando... Borrando usuario de prueba: ${user.email}',
          );
          await user.delete();
